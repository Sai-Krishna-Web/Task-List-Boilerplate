...
const TodoPublicList = props => {
   ...
   let oldestTodoId=numTodos?state.todos[numTodos-1].id:props.latestTodo?
                    props.latestTodo.id+1:0;
   let newestTodoId= numTodos?state.todos[0].id:props.latestTodo?
                      props.latestTodo.id:0;
  ...
  const loadOlder= async()=>{
       ...
       query getOldPublicTodos($oldestTodoId:Int!){
         todos(where :{is_public:{_eq:true}, id:{_lt:$oldestTodoId}}, limit:8, order_by:{created_at:desc}){
           id
           title
           created_at
           user{
